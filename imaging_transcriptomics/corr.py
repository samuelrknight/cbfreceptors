...
class CorrAnalysis:
    ...
    def gsea(self, gene_set="lake", outdir=None,
             gene_limit=5000):  # pragma: no cover, long to process (t > 1 h)
                                # - TESTED on run
        ...
        _boot_es = np.zeros((_origin_es.shape[0], 100))
        ...
        for i in range(_origin_es.shape[0]):
            _p_val[i] = np.sum(_boot_es[i, :] >= _origin_es[i]) / 100
