...
def extract_average(imaging_matrix):
    ...
    atlas_data = nib.load(
        ...
        / "data"
        / "Schaefer2018_100Parcels_7Networks_Xiao_2019_SubCorSeg_resampled_asl.nii.gz"
    ).get_fdata()
...
def load_gene_expression(regions="cort+sub"):
    ...
    expression_file_path = (
        Path(__file__).resolve().parent / "data" / "gene_expression_data_schaeferxiao122_2mm_resampled_asl.csv"
    )
    expression_data = pd.read_csv(expression_file_path, sep=",")
    #my_data_x = expression_data.iloc[0:82, 2:].to_numpy()
    my_data_x = expression_data.iloc[0:122, 2:].to_numpy()
    my_data = zscore(my_data_x, ddof=1)
    if regions == "cort+sub" or regions == "all":
        my_data = my_data
    elif regions == "cort":
        my_data = my_data[:100, 2:]
    return my_data
...
def get_geneset(gene_set: str):
    ...
    elif gene_set.lower() == "jorstad":
        return str(Path(__file__).parent / "data" /
                   "jorstad_2023_top_celltype_markers_diffstable.gmt")
    else:
        return gene_set
